...
async def notify_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in user_data_storage or not user_data_storage[user_id].get('queue'):
        await update.message.reply_text(
            "⚠️ Спочатку встановіть вашу чергу!\n\n"
            "Натисніть кнопку 🔢 Встановити чергу",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    if new_status:
        message = (
            ...
            "• За 15-25 хвилин до відключення ⚠️\n"
            "• Коли розпочнеться відключення 🔴\n"
            "• За 10-20 хвилин до відновлення ⏰\n"
            "• Коли відновиться електропостачання 🟢\n"
            "• При змінах у графіку 🔄\n\n"
            "💡 Щоб вимкнути - натисніть цю кнопку знову"
        )
    else:
        message = (
            "🔕 Сповіщення *вимкнено*\n\n"
            "Ви більше не отримуватимете автоматичні сповіщення.\n\n"
            "💡 Щоб увімкнути знову - натисніть цю кнопку"
        )
        ...
        for slot in slots:
            if '-' in slot and ':' in slot:
                ...
                try:
                    ...
                    # Сповіщення за 15 хвилин до ВІДКЛЮЧЕННЯ
                    # Перевіряємо інтервал замість точної хвилини
                    minutes_until_start = (start_hour * 60 + start_min) - (current_time.hour * 60 + current_time.minute)
                    
                    # За 15-25 хвилин до початку (враховуємо що перевіряємо кожні 10 хв)
                    if 15 <= minutes_until_start <= 25:
                        await context.bot.send_message(
                            chat_id=user_id,
                    ...
                    minutes_until_end = (end_hour * 60 + end_min) - (current_time.hour * 60 + current_time.minute)
                    
                    if 10 <= minutes_until_end <= 20:
                        await context.bot.send_message(
                            chat_id=user_id,
