            ...
            for row in rows[1:]:
                ...
                if re.match(r'\d{2}\.\d{2}\.\d{4}', date_cell):
                    ...
                    for i, cell in enumerate(cells[1:], 1):
                        time_slots = cell.get_text(strip=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_message = (
        "🔌 Вітаю! Я бот для відстеження відключень світла у Вінниці.\n\n"
        "📋 Доступні команди:\n"
...
async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = f"📅 *Графік відключень на {today}*\n\n"
    ...
    if today in schedule:
        if user_queue and user_subqueue:
            ...
            if queue_key in schedule[today] and subqueue_key in schedule[today][queue_key]:
                times = schedule[today][queue_key][subqueue_key]
        ...
        message += "📊 *Всі черги:*\n"
        for queue, subqueues in schedule[today].items():
            message += f"\n*{queue}*\n"
            for subqueue, times in subqueues.items():
                message += f"  • {subqueue}: {times}\n"
    else:
        message += "ℹ️ Графік на сьогодні ще не опубліковано."
...
def main() -> None:
    ...
    # Додаємо перевірку кожну хвилину
